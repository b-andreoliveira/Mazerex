...
protocol = "PR" #specify the protocol
print("this code is practice (PR) mode only!!!")
...
def append_rotation_live(ID_tag, protocol, wheel_counter): #define funtion for storing wheel data live
    ...
    dr_r = pd.DataFrame(rotation_list_live)

    if not os.path.isfile("rotations_live.csv"):
        df_r.to_csv("rotations_live.csv", encoding = "utf-8-sig", index = False)
    else:
        df_r.to_csv("rotations_live.csv", mode = "a+", header = False, encoding = "utf-8-sig", index = False)
...
def append_pellet_live(ID_tag, protocol, pellet_counter):#define function for storing pellet retrieval data
    ...
    df_p = pd.DataFrame(pellet_list_live)

    if not os.path.isfile("pellet_live.csv"):
        df_p.to_csv("pellet_live.csv", encoding = "utf-8-sig", index = False)
    else:
        df_p.to_csv("pellet_live.csv", mode = "a+", header = False, encoding = "utf-8-sig", index = False)
                    ...
                    if returned_A == False: #if animal has not yet returned from feeding area, perform these functions
                        ...
                        if dtState_A != dtLastState_A: 
                            ...
                            if wheel_counter_A >= limit_A: #when completes 1 full turn (wheel_counter = 160)
                                ...
                                append_rotation_live(IDtag_A, protocol, wheel_counter_A)
                            ...
                            append_pellet_live(IDtag_A, protocol, pellet_counter_A)
                ...
                while MODE_A == 2:
                    ...
                    if dtState_A != dtLastState_A:
                        ...
                        if wheel_counter_A >= limit_A: #when completes 1 full turn (wheel_counter = 1200)
                            ...
                            append_rotation_live(IDtag_A, protocol,turn_A)

                    elif GPIO.event_detected(read_FED_A): #detects signal coming from FED
                        pellet_counter_A += 1 #counts one pellet    
                    ...
                    if returned_B == False: #if animal has not yet returned from feeding area, perform these functions
                        ...
                        if dtState_B != dtLastState_B: 
                            ...
                            if wheel_counter_B >= limit_B: #when completes 1 full turn (wheel_counter = 1200)
                                ...
                                append_rotation_live(IDtag_B, protocol, wheel_counter_B)
                            ...
                            append_pellet_live(IDtag_B, protocol, pellet_counter_B)
                ...
                while MODE_B == 2:
                    ...
                    if dtState_B != dtLastState_B:
                        ...
                        if wheel_counter_B >= limit_B: #when completes 1 full turn (wheel_counter = 1200)
                            ...
                            append_rotation_live(IDtag_B, protocol, turn_B)

                    elif GPIO.event_detected(read_FED_B): #detects signal coming from FED
                        pellet_counter_B += 1 #counts one pellet    
...
while True: #infinite loop to constatly run these functions - these variables will inform the mean weight(last 12h) and pellet(last 3h)
    ...
    #print("Mouse A mean weight of last 12h: "+str(MEAN_WEIGHT_12hA)+" | "+str(datetime.now()))
    #print("Mouse B mean weight of last 12h: "+str(MEAN_WEIGHT_12hB)+" | "+str(datetime.now()))
    #print("Mouse A mean retireved pellets of last 3h: "+str(MEAN_PELLET_3hA)+" | "+str(datetime.now()))
    #print("Mouse B mean retireved pellets of last 3h: "+str(MEAN_PELLET_3hB)+" | "+str(datetime.now()))
    time.sleep(600) #waits 600 seconds (10 min) before updating and printing variables again
