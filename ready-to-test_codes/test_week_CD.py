...
IDtag_A = "202100251" #define tag ID for mouse A (converted from hexadecimal)
IDtag_B = "2006010137" #define tag ID for mouse B (converted from hexadecimal)
IDtag_C = "202100298" #define tag ID for mouse C (converted from hexadecimal)
IDtag_D = "137575399650" #define tag ID for mouse D (converted from hexadecimal)
IDtag_E = "???" #define tag ID for mouse E (converted from hexadecimal)
...
def RFID_check(X, protocol, IDtag, sec_IDtag): #define function to check ID of the animal
    ...
    if X == 'A':
        try:
            ...
            tag = 0
        print(tag)
        if tag == '00' or tag == IDtag or tag == '38723977519' or tag == sec_IDtag:

            if tag == IDtag:
                print("mouse A detected")
                which_mouse = "A"
                #ID_tag = "mouse A"
                append_RFID(which_mouse, tag, protocol)
                return True

            elif tag == sec_IDtag:
                print("mouse E deceted")
                which_mouse = "E"
                append_RFID(which_mouse, tag, protocol)

        else:
            print("not mouse A neither mouse E")
            return False
