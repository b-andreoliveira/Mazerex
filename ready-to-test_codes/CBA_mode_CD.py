...
def acquire_weight_pre(X): #define function to acquire and sotre weight data
    ...
    if X == 'C':
        ...
        for x in range(100): # 100 lines*120ms per line=12s of data 
            ...
            protocol = "CBA" #specifiy which protocol is being used - OG = original (base code)
                ...
                if returned_C == False: #if animal has not yet returned from feeding area, perform these functions
                    ...
                    if dtState_C != dtLastState_C: 
                        ...
                        if wheel_counter_C >= limit_C: #when completes 1 full turn (wheel_counter = 1200)
                            ...
                            if (turn_C % int(price_in_wheel_turns) == 0) and (turn_C != 0):: #each X revolutions
                                print("mouse C completed  " + str(price_in_wheel_turns) + " wheel turns, delivering pellet")
                                GPIO.output(writeFED_C, True) #sends output to FED - turnd FED motor on and makes pellet drop
                        else:
                            pass
                    
                    elif GPIO.event_detected(read_FED_C): #detects signal coming from the FED
                        GPIO.output(writeFED_C, False) #turns FED motor off
                ...
                if returned_D == False: #if animal has not yet returned from feeding area, perform these functions
                    ...
                    if dtState_D != dtLastState_D: 
                        ...
                        if wheel_counter_D >= limit_D: #when completes 1 full turn (wheel_counter = 1200)
                            ...
                            if turn_D % int(price_in_wheel_turns) == 0 and turn_D != 0: #each X revolutions
                                print("mouse D completed " + str(price_in_wheel_turns) + " wheel turns, delivering pellet")
                                GPIO.output(writeFED_D, True) #sends output to FED - turnd FED motor on and makes pellet drop
                        else:
                            pass
                    
                    elif GPIO.event_detected(read_FED_D): #detects signal coming from the FED
                        GPIO.output(writeFED_D, False) #turns FED motor off
