...
protocol = "PR" #specify the protocol -- Andre, use this in an if statement to select the protocol, so that all protocols are in one code.
'''
"OG" = original, refers to the base code (double_trouble_thread_NOHAT_A&B.py)
...
ser_A = serial.Serial()
ser_A.port = '/dev/ttyUSB2' #OpenScale serial port ALWAYS CHECK WHETHER THE PORT IS CORRECT!!!!!!!
ser_A.baudrate = 9600
...
ser_B = serial.Serial()
ser_B.port = '/dev/ttyUSB3' #OpenScale serial port ALWAYS CHECK WHETHER THE PORT IS CORRECT!!!!!!!
ser_B.baudrate = 9600
...
def RFID_check(X): #define function to check ID of the animal
    ...
    if X == 'A':
        try:
            ...
            tag = 0
        print(tag)
        if tag == '00' or tag == '137575399650' or tag == '38723977519':
        ...
        try:
            ...
            tag = 0
        print(tag)
        if tag == '00' or tag == '137575399602' or tag == '38723977519':
...
def acquire_weight_pre(X): #define function to acquire and sotre weight data
    ...
    if X == 'A':
        ...
        for x in range(100): # 100 lines*120ms per line=12s of data 
            ...
            protocol = "PR" #specifiy which protocol is being used - OG = original (base code)
        ...
        if MODE_A == 2:
            ...
            while MODE_A == 2:
                ...
                if returned_A == False: #if animal has not yet returned from feeding area, perform these functions
                    GPIO.output(writeFED_A, True) #as long as the animal has not returned, the Pi will always send signal for the FED to drop the pellet
                ...
                if returned_B == False: #if animal has not yet returned from feeding area, perform these functions
                    ...
                    if dtState_B != dtLastState_B: 
                        ...
                        if wheel_counter_B >= limit_B: #when completes 1 full turn (wheel_counter = 1200)
                            ...
                            if turn_B % turns == 0 and turn_B != 0: #each 10 revolutions
                                print("mouse B completed wheel turns, delivering pellet")
