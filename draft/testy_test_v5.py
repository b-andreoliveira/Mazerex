...
def animal_returned(): #define function t determine whether animal hs returned from feeding area and stores as boolean
    proximity_sensor = bus2.read_pin(2)
...
def bad_buzz(): #define funtion tp be executed when animal is heaveir than chosen weight treshold
    ...
    for _ in range(10):
        print("bad buzz")
        buzz.ChangeFrequency(heavier_buzz_1) #1st tone
        bus2.write_pin(11, 1) #turns red led ON
        time.sleep(0.1)
        buzz.ChangeFrequency(heavier_buzz_2) #2nd tone
...
def good_buzz(): #define function to be executed when animal is not heavier tan chosen weight treshold
    ...
    for _ in range (10):
        print("good buzz")
        buzz.ChangeFrequency(not_heavier_buzz_1) #1st tone
        bus2.write_pin(9, 1) #turns green led ON
        time.sleep(0.1)
        buzz.ChangeFrequency(not_heavier_buzz_2) #2nd tone
...
greenLED_ON = bus2.write_pin(9, 1)
greenLED_OFF = bus2.write_pin(9, 0)
greenLED_OFF; bus2.write_pin(9, 0) #double check if both operations are needed (they do the same)

bus2.set_pin_direction(11, 0)
redLED_ON = bus2.write_pin(11, 1)
redLED_OFF = bus2.write_pin(1, 0)
redLED_OFF; bus2.write_pin(11, 0) #double check if both operations are needed (they do the same)
...
dt = bus2.read_pin(1)
dtLastState = bus2.read_pin(1)

wheel_counter = 0;
cycle = 160; #calibration value (1200 for old wheels, 160 for new ones)
limit = cycle;
turn = 0;

#set pins (bus1 IOPi) for beam break (flying fish) proximity scanner
bus2.set_pin_direction(2, 1) #(pin, direction) #sets pin 1 on bus 1 to as an input pin (1 = input, 0 = output)
...
proximity_sensor = bus2.read_pin(2) #reas pin 1 (bus 1) of the IOPi as the sensor pin for the proximity sensor


#set pins (bus2 IOPi) for output to FED
bus1.set_pin_direction(1, 0) #output
FED_OFF = bus1.write_pin(1, 0) #sets pin 14 as low
FED_ON = bus1.write_pin(1, 1) #sets pin 14 as high
bus1.write_pin(1, 0)
...
bus1.set_pin_direction(9, 0) #pin 9 as output
air_puff_OFF = bus1.write_pin(9, 0) #set pin 9 to low/false
air_puff_ON = bus1.write_pin(9, 1) #set pin 9 to high/true
bus1.write_pin(9, 0)
...
while True:
    ...
    if MODE == 0:
        ...
        while MODE == 2:
            dtState = bus2.read_pin(1) #read input from running wheel
            ...
            if returned == False: #if animal has not yet returned from feeding area, perform these functions
                ...
                dtLastState = bus2.read_pin(1) #reset input from wheel 
